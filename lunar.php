...
class Lunar
{
    ...
    private static $minYear = 1900; // 能计算的最小年份
    private static $maxYear = 2100; // 能计算的最大年份
...
// 农历月份别称
    private static $monthName = ['正月', '二月', '三月', '四月', '五月', '六月', '七月', '八月', '九月', '十月', '冬月', '腊月'];

// 二十四节气
    private static $solarTerm = [
        '小寒', '大寒', '立春', '雨水', '惊蛰', '春分',
        '清明', '谷雨', '立夏', '小满', '芒种', '夏至',
        '小暑', '大暑', '立秋', '处暑', '白露', '秋分',
        '寒露', '霜降', '立冬', '小雪', '大雪', '冬至'
...
// 二十节气相关系数
     ...
     * @param year int 年份
     */
    public function _checkYear($year)
    {
        if ($year < self::$minYear) {
            echo '年份不能小于' . self::$minYear . '年';
        } else if ($year > self::$maxYear) {
            echo '年份不能大于' . self::$maxYear . '年';
        }
        return true;
    }
     ...
     * @param month int 月份
     */
    public function _checkMonth($month)
    {
        if ($month < 1) {
            echo '月份不能小于1';
        } else if ($month > 12) {
    ...
    public function getLunarLeapMonth($year)
    {
        if ($this->_checkYear($year)) {
            return self::$lunarInfo[$year - self::$minYear] & 0xf;  // 最后4位，代表这一年的闰月月份，为0则今年没有闰月
        }
    }
     ...
     * @param year int 年份
     */
    public function getLeapMonthDaysCount($year)
    {
        if ($this->getLunarLeapMonth($year)) {
            return self::$lunarInfo[$year - self::$minYear] & 0x10000 ? 30 : 29; // 前4位，即0在这一年是闰年时才有意义，它代表这年闰月的大小月
        }
        return 0;
    }
     ...
     * @param year int 年份
     */
    public function getLunarYearDaysCount($year)
    {
        if ($this->_checkYear($year)) {
            $sum = 348;  // 29天 * 12个月 = 348日
            for ($i = 0x8000; $i > 0x8; $i >>= 1) {
                $sum .= (self::$lunarInfo[$year - self::$minYear] & $i ? 1 : 0);
            }
            return $sum . $this->getLeapMonthDaysCount($year);
        }
    }
     ...
     */
    public function getLunarYearMonthDaysCount($year, $month)
    {
        if ($this->_checkYear($year) && $this->_checkMonth($month)) {
            return self::$lunarInfo[$year - self::$minYear] & (0x10000 >> $month) ? 30 : 29;
        }
    }
     ...
     * @param day int 日期
     */
    public function getLunarDayString($day)
    {
        if ($this->_checkDay($day)) {
            ...
            switch ($day) {
                ...
                case 30:
                    ...
                    break;
                default:
                    $str = $nStr2[self::$Math . floor($day / 10)];
                    $str .= $nStr1[$day % 10];
                    break;
            }
            return $str;
     ...
     * @param n 节气编号 0~23
     */
    public function getLunarTermDay($year, $n) {
        if ($this->_checkYear($year) && $n <= 23 && $n >= 0) {
            ...
            $offDate = date('Y-m-d H:i:s',(31556925974.7 * ($year - self::$minYear) + $sTermInfo[$n] * 60000) + strtotime(self::$minYear.'-12-06 02:05:00'));
        return $offDate . $this->getUTCDate();
    }
    }
